export default {
    '@tags': ['savedSearch'],
    'Validate Saved Search If Year Min > Year Max': (client) => {
        ...
        leadDetail.chooseOptionElement('@optionYear2017');

        leadDetail.chooseSelectElement('@yearMaxSelect');
        leadDetail.chooseOptionElement('@optionYear2016');

        leadDetail.submitAndSave();
        client.pause(waitForAPICallback);
        leadDetail.assert.cssProperty("@yearMaxSelect", "border-color", "rgb(255, 0, 0)");
        client.end();
    }
};